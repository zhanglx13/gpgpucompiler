...
/**
	* Performs privatization analysis on all Loops within a Program.  
	* Currently, only scalar variables are checked for privatization.
	* Variables that are defined before they are used within the loop 
	* are considered private to the loop.  
	* @deprecated
	*/
public class Privatization extends LoopAnalysisPass
{
  ...
  public void analyzeLoop(Loop loop)
  {
    ...
    /* upon seeing a define of an IDExpression already in used_set => do nothing
       not already in used_set => place in private_set   
       upon seeing a use of an IDExpression => place in used_set
    */
    ...
    for (;;)
    {
      ...
      Tools.printlnStatus(getPassName() + ": statement is " + stmt.toString(), 2);

      Set<Expression> stmt_use_set = DataFlow.getUseSet(stmt);
      Tools.printlnStatus(getPassName() + ":   use " + stmt_use_set.toString(), 2);

      Set<Expression> stmt_def_set = DataFlow.defines(stmt);
      Tools.printlnStatus(getPassName() + ":   def " + stmt_def_set.toString(), 2);
      ...
      for (Expression expr : stmt_def_set)
      {
    	...
    	Tools.printlnStatus(getPassName() + ":   private " + private_set.toString(), 2);
    }
    ...
    note.getMap().put("private", scalar_private);

    Tools.println("private list = " + scalar_private.toString(), 1);

		CompoundStatement parent_stmt = (CompoundStatement)(((Statement)loop).getParent());
    parent_stmt.addStatementBefore((Statement)loop, new DeclarationStatement(note));
  }
  ...
  public static Set<Expression> getPrivateSet(Annotation note)
  {
    if (note.getText().compareTo("private") == 0)
      return (Set<Expression>)note.getMap().get("private");
    else
